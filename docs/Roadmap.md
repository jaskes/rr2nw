...
## Текущий Windows-рубеж после восстановления Taxi

Runtime теперь загружает настоящий `SET_TAXI.SCI` каждого майского уровня и
владеет транзакционным переходом Taxi → Vehicle с полным rollback. Следующий
вертикальный срез:

1. выполнить F1 возле реального Taxi и открыть его настоящий целевой panel;
2. доказать отрисовку panel viewport без потери recovered Hardware input;
3. продолжить движение на заменившей машину наземной, воздушной или водной
   модели в течение ограниченной серии кадров;
4. вывести диагностику расстояния до ближайшего Taxi, чтобы ручная проверка
   воспроизводилась на конкретном уровне;
5. после этого переходить от границы Taxi/Vehicle к тяжёлому графу
   People/Tank/Bullet.
