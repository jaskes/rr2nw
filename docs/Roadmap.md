...
5. **closed:** add a bounded per-user profile catalog and in-frame selector;
   apply only after process restart, preserve CLI precedence and fail closed in
   safe mode or on corrupt/unresolvable input;
6. **closed for Windows 1.0:** deterministic 133-row pagination, existing-profile
   activation, bounded ASCII create/rename with focus/Esc cancellation,
   confirmed deletion, localization-ready compatibility categories and
   extracted-package validator identity are complete. A general UTF/IME text
   widget and translated strings require the separate localization owner. The
   remaining packaged Win10/Win11 campaign belongs to M6 acceptance.
...
of Tank/Cannon continuation restore are maintained. Level.06N command 34 now
owns its two-node checkpoint lifecycle and terminal authored handoff to
Level.01N with exact source rollback. Late Level.01D command 33 and the
unrecovered Destroyable behavior were kept distinct rather than being
conflated with Route capacity; command 33 is now closed as the exact May
RecruitCenter proximity watcher with transactional save/rollback. Destroyable
behavior remains separate. Level.07N is proved to author an empty ProjectTable
and no RecruitCenter.
...
latter proves
`ProjectG0 -> ProjectS04 -> ProjectS07 -> ProjectS10 -> ProjectS05 -> ProjectA26 -> ProjectS09 -> ProjectS06 -> ProjectAER04 -> ProjectAER06 -> ProjectAER08 -> ProjectAER10 -> ProjectAER00 -> ProjectAER16 -> ProjectAER21 -> ProjectS03 -> ProjectA27`,
...
S03 with another fresh slot-8 proof. S03 then atomically replaces the occupied
`HwzCln64..67` owners, retains two airplanes, four machine guns and two Tanks
as eight live objectives, reaches cumulative count sixteen and selects exact
A27. Its 30-owner graph, four-holder rollback/reapply and same-slot-8 fresh
restore are proved in all three configurations. A27 then executes its guarded
push-machine reached objective with the same actor retained as the failure-kill
guard, reaches cumulative count seventeen and retires with no successor,
Artifact or Portal. Terminal rollback/reapply and exact same-slot-8 fresh
restore close the installed Level.04D Actek center graph. That
...
slice through twenty ordinary results and its terminal all-kill row:
...
names and transient target reacquisition, A25's neutral Artefact, the AER
handoffs and G2's three real terminal kill targets. Public slots 1..8
then explicit slot-8 overwrite prove every result rollback/reapply and fresh
restore without a ninth-slot format, Artifact or Portal. G2 retires with no
successor and closes the installed Colony center graph. HWZ1 v1 saves migrate
...
implementation is approximately **87%** of the Windows-first 1.0 scope; strict
release readiness is approximately **74-78%** because a full campaign and the
...
| M1 modern Windows x86 | 96% | CMake/MSVC, real executable, all nine Levels, recovered software renderer and game loop, deterministic pending Portal Fountain admission with the archive unchanged | finish remaining campaign-owned callbacks and remove other narrow archive initialization debt |
| M2 Windows platform/stability | 89% | native window/input, focus neutralization, frame profiling, DPI-aware exclusive display ownership/recovery, stable skippable synchronous Level-intro presentation without recursive gameplay polling or palette-black flashes, exact SEH plus primary RTCHECK/assert and direct CRT `SIGABRT` minidump/manifest diagnostics, maintained XAudio2 2.9 cached one-shot/loops, bounded flags-1 briefing streams, symmetric listener/moving-Tank compatibility and direct occupied-Player-Vehicle pitch/lifecycle | broader moving-class/asymmetric/UI/non-WAV cinematic audio, prolonged multi-monitor/Win10/Win11 presentation soak, invalid-parameter/purecall/tool-exit classification and sanitizer coverage |
...
3. widen the closed listener/moving-Tank/Player-Vehicle/briefing-stream audio
   ...
   classify the remaining invalid-parameter/purecall/tool fatal paths and finish
