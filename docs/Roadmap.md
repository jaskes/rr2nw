...
| M4 save/timing/VFS | 87% | versioned 17-owner LCN1, atomic same/cross-Level load, CTJ1, RNG split, deterministic VFS/content identity, RPH1 content-bound per-tick Vehicle/CLK1/RNG hashes, live bounded Windows fixed-step cadence with zero-to-four Session/Vehicle ticks per presentation, focus/shell reset, post-present transactions, LCN1 epoch rebase and a 27/27 installed-Level production gate | complete-world replay hashes, legacy import breadth and long-session wrap/drift proof; renderer interpolation remains optional |
