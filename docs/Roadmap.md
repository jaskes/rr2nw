...
6. **closed for Windows 1.0:** deterministic 133-row pagination, existing-profile
   activation, confirmed deletion, localization-ready bounded compatibility
   categories and extracted-package validator identity are complete. Safe
   create/rename is deferred until the shell owns validated text/IME/caret and
   commit/cancel input; translated strings require the separate localization
   owner. The remaining packaged Win10/Win11 campaign belongs to M6 acceptance.
...
| M5 modding | 92% | discovery, dependencies/conflicts, deterministic mount order, validator, data/script overlays, bounded player profiles, restart-only in-frame selector, 128-candidate pagination, existing-profile activation/confirmed deletion, bounded compatibility keys and extracted-package identity parity | safe create/rename awaits a real text-input owner; translated strings and final Win10/Win11 package campaign remain |
| M6 release candidate | 45% | CI configurations, schema-2 reproducible package with exact EXE/validator/PDB/MAP identity, portable first-run retail selection, diagnostics and extensive automated matrices | exact clean RC artifact, installer/importer decision, Win10 full campaign, Win11 extended pass and final support docs |
