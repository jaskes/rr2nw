...
| M4 save/timing/VFS | 82% | versioned 17-owner LCN1, atomic same/cross-Level load, CTJ1, RNG split, deterministic VFS/content identity, RPH1 content-bound per-tick Vehicle/CLK1/RNG hashes, bounded integer cadence owner with real dense/four-tick submissions, stall/focus telemetry, explicit Session tick and complete-frame seams | whole-world dynamic-clock rebase and production scheduler activation, complete-world replay hashes, legacy import breadth and long-session timing proof |
